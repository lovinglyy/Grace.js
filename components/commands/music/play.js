  ...
  */
  cmd: async function( msg, argSeparator, redisClient, youtubeAPI  ) {
    ...
    if ( youtubeLinkPos !== -1 ) {
      ...
      let searchResults = await libs.music.searchYoutubeSong( msg, youtubeAPI, idFromLink );
      if ( !searchResults ) return msg.reply( 'no results found :p' );
      [ songId, songTitle ] = searchResults;

    } else if ( isNaN ( Number( singleArgument ) ) ) {
      let searchResults = await libs.music.searchYoutubeSong( msg, youtubeAPI, singleArgument );
      if ( !searchResults ) return msg.reply( 'no results found :p' );
      [ songId, songTitle ] = searchResults;
      ...
      let song = libs.music.findSongByIndex(userPlaylist, songNumber);
      if ( !song ) return msg.reply( 'that song number isn\'t in your playlist.' );
      let songTitlePos = song.indexOf( '!SongTitle' );
    ...
    if ( !graceVCInGuild ) {
      ...
      if ( !(msg.guild.voiceConnection.dispatcher) && msg.guild.me.speaking === false ) {
    ...
    function dispatcherOnEnd(dispatcher) {
      dispatcher.on('end', async (reason) => {
        ...
        if ( libs.music.checkForSomeoneInVC( msg.guild.voiceConnection.channel.members ) === false ) {
          ...
          const hgetAsync = promisify( redisClient.hget ).bind( redisClient );
          let guildPlaylist = await hgetAsync( msg.guild.id, "guildPlaylist" );
          ...
          let nextSong = guildPlaylist.substring( 0, 11 );

          let songTitle;
          let searchResults = await libs.music.searchYoutubeSong( msg, youtubeAPI, nextSong );
          [ _, songTitle ] = searchResults;

          let newStream = ytdl(`https://www.youtube.com/watch?v=${nextSong}`, { filter : 'audioonly' });
          let newDispatcher = msg.guild.voiceConnection.playStream(newStream, streamOptions);
