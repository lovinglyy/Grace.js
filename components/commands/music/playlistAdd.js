...
module.exports = {
	...
	*/
	cmd: async function( msg, argSeparator, redisClient, youtubeAPI  ) {
		...
		let search = ( youtubeLinkPos === -1 ) ? singleArgument : msg.content.substring( youtubeLinkPos + 20, youtubeLinkPos + 31 );
		if (!search) return msg.reply( 'please tell me a valid youtube link or song name! *grrr*' );

		let songId;
		let songTitle;
		let searchResults = await libs.music.searchYoutubeSong( msg, youtubeAPI, search );
