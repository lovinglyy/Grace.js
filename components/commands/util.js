 ...
 * this can be used for a better case insensitive reliability, however with some local tests, it appeared
 * to decrease a huge amount of performance.
 */
 function compareStrings( str1, str2 ) {
	...
	for ( let i = 0 ; i < smallerLength + 1 ; i++ )
		if ( str1[i] === str2[i] && str1[i] !== ' ' && str2[i] !== ' ' ) equalChars++;
 ...
 */
 function randomStringCompare( str1, str2 ) {	
	...
	for ( let i = 0 ; i < ~~(smallerLength/2) ; i++ ) {
		let rndChar = ~~(Math.random() * smallerLength - 1);
...
function getMember( search, msg, errorReply ) {
	...
	if ( !usernameSearch ) {		
		similarMembers.sort( function( a, b ) { return b[0] - a[0]; } );
		...
		if (errorReply) msg.channel.send( `${msg.author}, sorry but I couldn't find the specified member :c`, {embed: embed} );
		return false;
	}
