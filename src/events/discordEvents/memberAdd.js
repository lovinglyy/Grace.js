module.exports = class {
  ...
  start() {
    this.client.on('guildMemberAdd', async (member) => {
      ...
      const welcomeMsg = await this.asyncRedis.hget(`guild:${member.guild.id}`, 'welcome');
      if (!welcomeMsg) return;
      const welcomeChannelID = await this.asyncRedis.hget(`guild:${member.guild.id}`, 'welcomeChannel');
